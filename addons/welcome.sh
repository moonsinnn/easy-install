...
# Shadowsocks WS
status="$(systemctl show xray@shadowsocksws --no-page)"
status_text=$(echo "${status}" | grep 'ActiveState=' | cut -f2 -d=)
if [ "${status_text}" == "active" ]; then
        echo -e "SS WS           : "$green"Aktif"$NC""
else
        echo -e "SS WS           : "$red"Tidak Aktif (Error)"$NC""
fi

# Shadowsocks GRPC
status="$(systemctl show xray@shadowsocksgrpc --no-page)"
status_text=$(echo "${status}" | grep 'ActiveState=' | cut -f2 -d=)
if [ "${status_text}" == "active" ]; then
        echo -e "SS GRPC         : "$green"Aktif"$NC""
else
        echo -e "SS GRPC         : "$red"Tidak Aktif (Error)"$NC""
fi

# Shadowsocks WS 2022
status="$(systemctl show xray@ssblakews --no-page)"
status_text=$(echo "${status}" | grep 'ActiveState=' | cut -f2 -d=)
if [ "${status_text}" == "active" ]; then
        echo -e "SS2022 WS	: "$green"Aktif"$NC""
else
        echo -e "SS2022 WS	: "$red"Tidak Aktif (Error)"$NC""
fi

# Shadowsocks GRPC 2022
status="$(systemctl show xray@ssblakegrpc --no-page)"
status_text=$(echo "${status}" | grep 'ActiveState=' | cut -f2 -d=)
if [ "${status_text}" == "active" ]; then
        echo -e "SS2022 GRPC	: "$green"Aktif"$NC""
else
        echo -e "SS2022 GRPC	: "$red"Tidak Aktif (Error)"$NC""
fi
