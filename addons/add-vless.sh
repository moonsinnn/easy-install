...
},{"email": ''"'"$user"'"'',"id": ''"'"$uuid"'"'', "level": 0, "alterId": 0' /usr/local/etc/xray/vlesswstls.json
sed -i '/#membervlessgrpc$/a\### '"$user $exp"'\
},{"email": ''"'"$user"'"'',"id": ''"'"$uuid"'"'', "level": 0' /usr/local/etc/xray/vlessgrpc.json

### Creator Link
# Vless
vlesswstls="vless://${uuid}@${domain}:$porttls?path=/netzvless${fix}security=tls${fix}encryption=none${fix}type=ws#${user}"
vlessgrpc="vless://${uuid}@${domain}:$porttls?mode=gun${fix}security=tls${fix}encryption=none${fix}type=grpc${fix}serviceName=netzvlessgrpc#${user}"
# CLI
vlesswstlscli="vless://${uuid}@${domain}:$porttls?path=/netzvless&security=tls&encryption=none&type=ws#${user}"
vlessgrpccli="vless://${uuid}@${domain}:$porttls?mode=gun&security=tls&encryption=none&type=grpc&serviceName=netzvlessgrpc#${user}"
...
echo -e "=> WS TLS" : "$vlesswstlscli" | tee -a /root/akun/vless/$user.txt
echo -e | tee -a /root/akun/vless/$user.txt
echo -e "=> GRPC" : "$vlessgrpccli" | tee -a /root/akun/vless/$user.txt
echo -e | tee -a /root/akun/vless/$user.txt
