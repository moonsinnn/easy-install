...
# Shadowsocks
data=($(cat /usr/local/etc/xray/shadowsocksws.json | grep '^###' | cut -d ' ' -f 2))
now=$(date +"%Y-%m-%d")
for user in "${data[@]}"; do
        exp=$(grep -w "^### $user" "/usr/local/etc/xray/shadowsocksws.json" | cut -d ' ' -f 3)
        d1=$(date -d "$exp" +%s)
        d2=$(date -d "$now" +%s)
        exp2=$(((d1 - d2) / 86400))
        if [[ $exp2 == "0" ]]; then
                sed -i "/^### $user $exp/,/^},{/d" /usr/local/etc/xray/shadowsocksws.json
                sed -i "/^### $user $exp/,/^},{/d" /usr/local/etc/xray/shadowsocksgrpc.json
        fi
done
...
# Shadowsocks 2022
data=($(cat /usr/local/etc/xray/ssblakews.json | grep '^###' | cut -d ' ' -f 2))
now=$(date +"%Y-%m-%d")
for user in "${data[@]}"; do
        exp=$(grep -w "^### $user" "/usr/local/etc/xray/ssblakews.json" | cut -d ' ' -f 3)
        d1=$(date -d "$exp" +%s)
        d2=$(date -d "$now" +%s)
        exp2=$(((d1 - d2) / 86400))
        if [[ $exp2 == "0" ]]; then
                sed -i "/^### $user $exp/,/^},{/d" /usr/local/etc/xray/ssblakews.json
                sed -i "/^### $user $exp/,/^},{/d" /usr/local/etc/xray/ssblakegrpc.json
        fi
done
