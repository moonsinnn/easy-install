...
},{"email": ''"'"$user"'"'',"id": ''"'"$uuid"'"'', "level": 0, "alterId": 0' /usr/local/etc/xray/vmesswstls.json
sed -i '/#membervmesswsnontls$/a\### '"$user $exp"'\
},{"email": ''"'"$user"'"'',"id": ''"'"$uuid"'"'', "level": 0, "alterId": 0' /usr/local/etc/xray/vmesswsnontls.json
sed -i '/#membervmessgrpc$/a\### '"$user $exp"'\
},{"email": ''"'"$user"'"'',"id": ''"'"$uuid"'"'', "level": 0' /usr/local/etc/xray/vmessgrpc.json
...
echo -e PORT NONTLS : 80/8080/2086/8880/2052/2082/2095 | tee -a /root/akun/vmess/$user.txt
...
echo -e "=> NO TLS : worryfree" | tee -a /root/akun/vmess/$user.txt
